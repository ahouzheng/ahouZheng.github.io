...
- 为何不允许null value
在HashMap中，操作时单线程的，当get到null值时，可以通过containsKey()来判断是存入了null值还是key不存在，而ConcurrentHashMap是多线程操作的当get到null值时，调用containsKey()前可能有其他线程改变了状态，导入无法准确判断是存入了null值还是key不存在，消除不了歧义
- 为何不允许null key
理论上ConcurrentHashMap中存储null key是没有问题的，但这会给map的代码编写带来麻烦，需要在很对地方判断null，并且为null建立映射本身是不合理的
