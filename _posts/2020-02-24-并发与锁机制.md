...
**适应性自旋锁：**自旋的时间不再固定，而是由前一次在同一个锁上的自旋时间和锁对象来决定。
...
线程持有锁的时间比较长时可以使用公平锁，这样对吞吐量的影响并不大  
...
## synchronized
**加锁方式：** 如果是对非静态方法同步，作用对象为调用该方法的实例  
...
从可见性角度看，volatile变量写操作相当于退出同步代码块，读操作相当于进入同步代码块  
volatile并不能像synchronized一样绝对的保证线程安全  
volatile只有在满足下面条件时，才能安全使用，否则需要加锁：  
...
- lockInterruptibly 锁定对象，允许在获得锁的过程中响应中断
...
**通常使用的线程池有四种：**  
- newCachedThreadPool    大小不受限，但核心线程数为0
- newFixedThreadPool     大小一定，无可用线程时，需阻塞等待可用线程，无限队列
- newSingleThreadExecuter  创建单线程，任务需要按序执行
- newScheduledThreadPool   支持定时及周期性任务

**线程池重要参数：**  
ThreadPoolExecutor类有七个重要的参数  
- corePoolSize，核心线程数，线程池维护的最少线程数量，即使这些线程空闲超时也不会被销毁，除非allowCoreThreadTimeout设置为true，在线程数小于该值时，任务到来时，即使有空闲线程也新建线程来执行任务。  
...
- workQueue，工作队列，任务提交后，若当前线程数已经达到corePoolSize,会先放入工作队列，然后再由线程执行。若队列已满，则新建线程，直到到达最大线程数。  
...
**相关方法：**  
- add(E e)，向队列中放入数据，不会被阻塞，成功返回true，队列满抛出异常
- offer(E e)，向队列中放入数据，不会被阻塞，成功返回true，队列满返回false
- offer(E e, long timeout, TimeUnit unit), 当队列满时，允许被阻塞设置的时间，然后成功添加返回true，否则返回false
- put(E e)，向队列放入数据，会被阻塞直至数据放入，但锁允许中断，不返回值
- remove() 从对列取数据，不会被阻塞，成功返回队列头部数据，队列为空时抛出异常
- poll()，从队列取数据，失败返回null，不阻塞d
- poll(long timeout, TimeUnit unit)，队列为空时，允许被阻塞设定时间，然后失败返回false
- take()，从队列取出数据，会被阻塞直到成功取到数据，锁允许中断  

ArrayBlockingQueue，添加和取出操作使用同一把锁，不允许添加和取出并行  
LinkedBlockingQueue，添加和取出操作使用各自的锁，允许添加和取出操作并行  
**影响分析：**
LinkedBlockingQueue在并发读写时吞吐量会更高，本质上ArrayBlockingQueue的循环数组和LinkedBlockingQueue的链表都是头尾分离的，均可支持同时读写
一个小问题：当队列满时put会阻塞，若同时支持读写，可能会导致乱序，如发起一个put任务a时，判断队列满，执行await前，有一个take任务取走了一个元素，那任务a仍然会await，而之后的put任务反而执行成功
...
这里setnx与expire不是原子操作，如果在加锁之后线程退出，没有设置过期，那锁将无法自动超时释放。
...
协程是轻量级的线程，一个线程可以对应多个协程，协程之间的切换在用户态进行，协程是对应函数执行过程中的切换，同一个线程的协程执行是交替的，串行的。
...
#### 线程状态
**参考文章：[article](https://www.cnblogs.com/Tiancheng-Duan/p/13955212.html)**
...
## NIO
**参考文章：[article1](https://www.zhihu.com/question/19732473)，[article2](https://www.jianshu.com/p/5bb812ca5f8e)，[article3](https://blog.csdn.net/weixin_37778801/article/details/86699341)，[article4](https://blog.csdn.net/jike11231/article/details/109385845?spm=1001.2101.3001.6650.1&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-109385845-blog-115129800.pc_relevant_aa&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-109385845-blog-115129800.pc_relevant_aa&utm_relevant_index=2)**  
...
- 同步非阻塞：不会一直呆在水边**等待**是否烧开，可以去做些其他事，但要隔一段时间**观察**一下结果
