...
#### Spring为什么采用三级缓存
**参考文章：[article1](https://blog.csdn.net/qq_37699336/article/details/124421852?spm=1001.2101.3001.6650.1&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-124421852-blog-124052220.pc_relevant_aa&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-124421852-blog-124052220.pc_relevant_aa&utm_relevant_index=2)，[article2](https://blog.csdn.net/qq_33468007/article/details/107814203?spm=1001.2101.3001.6650.1&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-107814203-blog-108067624.pc_relevant_multi_platform_whitelistv3&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-107814203-blog-108067624.pc_relevant_multi_platform_whitelistv3&utm_relevant_index=2)**  
spring采用三级缓存主要是为了解决IOC时循环依赖和AOP时循环依赖的问题  
- 一级缓存中存储了就绪的最终bean
- 二级缓存中存储了经过代理后的bean
- 三级缓存中存储了实例化之后的半成品bean
如果不支持循环依赖，一级缓存即可，将bean初始化完成放入缓存即可  
当出现循环依赖时，没办法直接将bean创建并初始化，因为在注入属性时会循环锁死，所以需要加一层缓存存放未进行依赖注入的bean，放到三级缓存中，在依赖注入阶段从三级缓存获取依赖的对象即可，此时两级缓存可解决该问题  
当循环依赖的bean又通过AOP增强时，两级缓存也会出现问题，当一级缓存中就绪的A1依赖A2时，我们需要代理原始bean A1来增强他，这是给他注入的是原始的A2，然后代理A2时可注入代理后的A1，这是出现的问题是最终增强的A1使用了原始的A2，A1不是我们AOP后需要的bean，所以需要在中间插入二级缓存，存储增强后的代理对象  
...
## Nginx
**参考文章：[article1](https://www.php.cn/nginx/423510.html), [article2](https://developer.51cto.com/art/201910/605110.htm)**    
...
## Dubbo Dubbox
**参考文章：[dubbo负载均衡与容错](https://www.cnblogs.com/xhj123/p/9087532.html), [article](https://blog.csdn.net/GoldWashing/article/details/82316089)**  
...
## RPC
**参考文章：[article](https://blog.csdn.net/weixin_45191798/article/details/97620171)**    
RPC （远程过程调用）  
客户端调用方法时，先将相关参数(代表函数的call ID和函数参数)序列化，然后通过网络传输到服务端，在服务端反序列化后，调用目标方法进行处理，处理的结果再序列化后返回到调用端。
...
#### RPC与REST
**参考文章：[article](https://blog.csdn.net/u014590757/article/details/80233901?depth_1-utm_source=distribute.pc_relevant.none-task&utm_source=distribute.pc_relevant.none-task)**  
...
#### ZAB协议
**参考文章：[article](https://baijiahao.baidu.com/s?id=1718416446363405796&wfr=spider&for=pc)**  
Zookeeper通过ZAB协议实现分布式数据一致性，ZAB协议工作在两种模式下，分别是恢复模式和广播模式
- 恢复模式，当系统启动时，或出现leader崩溃，中断等异常情况时，ZAB会工作在恢复模式，这种模式下首先会选举出leader，然后其他服务器与leader进行数据同步，在leader选举阶段，所有的follower节点都向其他节点发起投票，第一轮所有机器都会投自己，节点收到投票后会先比较epoch选出epoch最大的节点，epoch相等时比较Zxid（事务id）最大的节点（说明节点的事务是比较新的），都相等时选择serverId最大的节点
...
#### Zookeeper持久化
**参考文章：[article1](https://artisan.blog.csdn.net/article/details/109792293?spm=1001.2101.3001.6661.1&utm_medium=distribute.pc_relevant_t0.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-109792293-blog-121255054.pc_relevant_aa&depth_1-utm_source=distribute.pc_relevant_t0.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-1-109792293-blog-121255054.pc_relevant_aa&utm_relevant_index=1)，[article2](https://blog.csdn.net/lxlneversettle/article/details/121255054)**  
zookeeper有两种持久化机制  
- 事务日志，zookeeper在写操作前会记录日志（WAL）
- 快照，当事务日志累计大指定数目时对全量数据生成快照
恢复数据时向从快照恢复数据，然后再通过事务日志做增量恢复

#### Zookeeper实现数据一致性的算法
**参考文章：[article1](https://www.cnblogs.com/jelly12345/p/15603515.html)，[article2](https://juejin.cn/post/6844903919026438158)**  
zookeeper写入实现的是强一致性，读取时顺序一致性  
zookeeper读取的顺序一致性保证是服务端和客户端配合实现的，客户端会记录它见到的最大z
xid，在请求server时，如果该zxid比server端的最大zxid要大，则拒绝请求，客户端重连到其他节点来请求，以此保证客户端不会读到比之前旧的数据  
...
## Maven
**参考文章：[article](https://blog.csdn.net/happydecai/article/details/80492212)**  
...
#### Maven依赖范围
**参考文章：[article](https://www.sohu.com/a/212704477_100012573)**  
...
- 编译检查，通过代码里标识的元数据让编译器在编译阶段进行程序检查
