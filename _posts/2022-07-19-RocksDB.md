...
### kafka producer消息发送
**参考文章：[article](https://www.likecs.com/show-204370862.html)**  
kafka 生产端组件有
- KafkaProducer
KafkaProducer 是一个生产者客户端的进程，通过该对象启动生产者来发送消息。  
- RecordAccumulator
RecordAccumulator 是一个记录收集器，用于收集客户端发送的消息，并将收集到的消息按partition粒度暂存到客户端缓存中。    
- Sender
...
- 计算消息所属的分区，将消息缓存记录累加器中，每个分区的数据存在一个双端队列里
- 当记录累加器达到一定数量的消息时批量发送到broker
