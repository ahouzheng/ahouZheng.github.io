...
- 当记录累加器中的消息达到一定大小后批量发送到broker，单个分区累加数据的大小限制默认是16KB，该值较小时，消息发送更及时，但吞吐量会降低，更大时，吞吐量增高，但消息发送延时会增加，内存占用也会增加，数据发送是按broker维度进行发送的，同一个broker的多个partition数据会在同一个连接上发送，而不是每个partition单独使用一个连接
