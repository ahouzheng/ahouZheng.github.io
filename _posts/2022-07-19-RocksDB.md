...
presto资源组用于对不同分组的查询进行一定程度的资源隔离，用于限制查询的资源使用，presto会根据查询类型、用户和标签将查询路由到不同的group，而不同的group具有一定的资源限制，如并发查询数，排队查询数，允许使用的内存、cpu等资源，通过这些限制，保证集群中不同查询的资源使用更加合理，避免资源挤占  
