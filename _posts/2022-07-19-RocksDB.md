...
每个parititon对应一个Deque\<ProducerBatch\>，当发送一个消息时取出队列尾端的batch判断是否还能接收消息，可以就放进去，不行就新建一个batch，新建时若消息大小超过batch size则按消息大小分配batch内存，不超过则按batch size分配，batch size默认是16KB，该值较小时，消息发送更及时，但吞吐量会降低，更大时，吞吐量增高，但消息发送延时会增加，内存占用也会增加，数据发送是按broker维度进行发送的，同一个broker的多个partition数据会在同一个连接上发送，而不是每个partition单独使用一个连接  
